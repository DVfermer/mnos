...
#if defined(NDEBUG)
# error "MNOS Core cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "MNOS Signed Message:\n";
...
{
    if (nPrevHeight == 0)
      return 840000 * COIN;  // 4% Premine
    else if (nPrevHeight >= 1 && nPrevHeight <= 14)
      return 1 * COIN;  // Instamine Protection For The First Half Hour (1 COIN/Block)
    else if (nPrevHeight >= 15 && nPrevHeight <= 59)
      return 2 * COIN; // Instamine Protection For The Second Hour (2 COIN/Block)
    else if (nPrevHeight >= 60 && nPrevHeight <= 719)
      return 4 * COIN; // Instamine Protection For Day 1 (4 COIN/Block)
    else if (nPrevHeight >= 720 && nPrevHeight <= 1439)
      return 6 * COIN; // Instamine Protection For Day 2 (6 COIN/Block)
    else if (nPrevHeight >= 1440 && nPrevHeight <= 5039)
      return 8 * COIN; // Instamine Protection For Day 3 - 7 (8 COIN/Block)

    // LogPrintf("height %u diff %4.2f reward %d\n", nPrevHeight, dDiff, nSubsidyBase);
    CAmount nSubsidy = 16 * COIN;

    // quarterly decline of production by ~10% per quarter, projected ~21M coins max by year 2067+.
    if(nPrevHeight >= consensusParams.nSubsidyHalvingInterval * 27)
        nSubsidy = 1 * COIN; //Minimum block rewards is 1 COIN, about 27 quarters later (~ 1,749,600 blocks)
    else
    {
...
static bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck = false)
{
    ...
    // MNOS : MODIFIED TO CHECK MASTERNODE PAYMENTS AND SUPERBLOCKS
    ...
    if (!IsBlockValueValid(block, pindex->nHeight, blockReward, strError)) {
        return state.DoS(0, error("ConnectBlock(MNOS): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }

    if (!IsBlockPayeeValid(block.vtx[0], pindex->nHeight, blockReward)) {
        mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
        return state.DoS(0, error("ConnectBlock(MNOS): couldn't find masternode or superblock payments"),
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // MNOS : CHECK TRANSACTIONS FOR INSTANTSEND
    ...
    if(sporkManager.IsSporkActive(SPORK_3_INSTANTSEND_BLOCK_FILTERING)) {
        ...
        BOOST_FOREACH(const CTransaction& tx, block.vtx) {
            ...
            BOOST_FOREACH(const CTxIn& txin, tx.vin) {
                ...
                if(instantsend.GetLockedOutPointTxHash(txin.prevout, hashLocked) && hashLocked != tx.GetHash()) {
                    ...
                    LOCK(cs_main);
                    mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
                    return state.DoS(0, error("CheckBlock(MNOS): transaction %s conflicts with transaction lock %s",
        ...
        LogPrintf("CheckBlock(MNOS): spork is off, skipping transaction locking checks\n");
    }
