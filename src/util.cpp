...
const char * const BITCOIN_CONF_FILENAME = "mnos.conf";
const char * const BITCOIN_PID_FILENAME = "mnosd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "mnos" is a composite category enabling all MNOS-related debug output
            if(ptrCategory->count(string("mnos"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\MNOS
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\MNOS
    // Mac: ~/Library/Application Support/MNOS
    // Unix: ~/.mnos
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "MNOS";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/MNOS";
#else
    // Unix
    return pathRet / ".mnos";
#endif
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty mnos.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override mnos.conf
