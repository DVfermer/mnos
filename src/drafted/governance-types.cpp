...
GovernanceObjectType GovernanceStringToType(std::string strType) {
    ...
    if(strType == "MNOSNetwork") type = MNOSNetwork;
    if(strType == "MNOSNetworkVariable") type = MNOSNetworkVariable;
...
std::string GovernanceTypeToString(GovernanceObjectType type) {
    ...
    if(type == MNOSNetwork) s = "MNOSNetwork";
    if(type == MNOSNetworkVariable) s = "MNOSNetworkVariable";
