...
MNOS Core is a multithreaded application, and deadlocks or other multithreading bugs
...
- ThreadSocketHandler : Sends/Receives data from peers on port 6555.
...
- ThreadRPCServer : Remote procedure call handler, listens on port 6556 for connections and services them.
...
However, in open source software such as MNOS Core, where everyone uses
...
pay attention to for reviewers of MNOS Core code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: MNOS Core uses tinyformat, which is type safe. Leave them out to avoid confusion
